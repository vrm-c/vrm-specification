...
プロシージャル: システムから自動的に値が生成されることが想定されています。
...
* `blink` と同時に `lookAt` が適用される => 目が瞼を貫通してしまう

などです。
これらを防御するために、 `happy` 中はリップシンクをしないというような設定が必用です。
overrideMouth, overrideBlink, overrideLookAt で設定できます。

それぞれの override プロパティは、以下のプロシージャル表情に対して作用します:

| プロパティ       | 作用する表情                                         |
|:-----------------|:-----------------------------------------------------|
| `overrideMouth`  | LipSync `aa`, `ih`, `ou`, `ee`, `oh`                 |
| `overrideBlink`  | 瞬き `blink`, `blinkLeft`, `blinkRight`              |
| `overrideLookAt` | LookAt `lookUp`, `lookDown`, `lookLeft`, `lookRight` |
