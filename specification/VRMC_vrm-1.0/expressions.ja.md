...
オーバーライドの影響を与える表情にisBinaryが指定されている場合、その表情の出力値である「0.5以上なら1、0.5未満なら0」を評価し、その値で影響を与えなければいけません（MUST）。
...
オーバーライドの影響を受ける表情にisBinaryが指定されている場合、受けている影響が0.0より大きければその表情は完全に抑制されなければいけません（MUST）。

> これは、オーバーライドの影響を受ける表情が0もしくは1以外の値で発現することを防ぐための仕様です。
